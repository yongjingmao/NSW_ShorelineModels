    ...
    for i=1:Ntr % for all transects
        ...
        if strcmp(transects(i).model_type,'full model') || strcmp(transects(i).model_type,'cross-shore only') || strcmp(transects(i).model_type,'rate only')
            ...
            description=['<html><table border="1">', ...
                ...
                '<tr><td>CA  (eq. shoreline change time scale)    [days] </td><td>',num2str(nanmean(ca(i,:)),4)         ,'</td></tr>', ...
                '<tr><td>CE  (eq. shoreline change erosion scale) [m]    </td><td>',num2str(nanmean(ce(i,:)),4)         ,'</td></tr>', ...
                '<tr><td>PHI (eq. background wave height)         [m]    </td><td>',num2str(nanmean(phi(i,:)),4)        ,'</td></tr>', ...
