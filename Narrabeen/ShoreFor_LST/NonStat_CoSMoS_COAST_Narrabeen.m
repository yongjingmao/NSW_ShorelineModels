...
ca00 = 5e-8*sqrt(2229)*24*3600; % accretion rate,  [m^-1.4 day^-1]
...
% noise parameter
lb_sigma=0.001;   ub_sigma=2;  % adative noise spread [units vary but nominally meters]
...
%da_var_noise_fac  =[  1.2     0.5    1e-4   5e-5   2e-4  0.005    0.01    0.025   0.0025]; % noise factor coefficient (Stationary)
da_var_noise_fac  =[  1.2     0.5    2e-3   1e-3   1e-2  0.005    0.01 0.025   0.0025]; % noise factor coefficient (Non-stationary)
...
for n=1:len-1 % for all time steps
    ...
    if t1(n+1)>=tforecast
        ...
        if Nonstationary
            ...
            ce = CE_cali(:, :, n);
            phi = PHI_cali(:, :, n);
            % ca = repmat(CA_cali(:, n), 1, Nens) +da_var_noise_fac(strcmp(da_vars,'ca'))*repmat(NOISE_FAC,1,Nens).*NOISE.*randn(Ntr,Nens);
            % ce = repmat(CE_cali(:, n), 1, Nens) +da_var_noise_fac(strcmp(da_vars,'ce'))*repmat(NOISE_FAC,1,Nens).*NOISE.*randn(Ntr,Nens);
            % phi = repmat(PHI_cali(:, n), 1, Nens)+da_var_noise_fac(strcmp(da_vars,'phi'))*repmat(NOISE_FAC,1,Nens).*NOISE.*randn(Ntr,Nens);
        end
