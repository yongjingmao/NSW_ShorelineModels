...
for i=1:length(ids)
    ...
    if (strcmp(transects(id_tr).model_type,'full model') || strcmp(transects(id_tr).model_type,'cross-shore only') || strcmp(transects(id_tr).model_type,'rate only')) && ~bool_no_prediction(id_tr)
        ...
        han1=plot(t,Hs(id_tr,:,1),'-b',t(idw),Hs(id_tr,idw),'ro'); set(han1,'MarkerFaceColor','r','MarkerSize',10);  set(gca,'XTick',linspace(t(1),t(min(n+tmore,length(t))),12),'Xlim',[t(1) t(min(n+tmore,length(t)))]); datetick('x','keeplimits','keepticks');
        ...
        vmin=min(YLSTCI1(id_tr,~isnan(t_output))); vmin=min(vmin,min(YLST(id_tr,:))); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        vmax=max(YLSTCI2(id_tr,~isnan(t_output))); vmax=max(vmax,max(YLST(id_tr,:))); if vmax<0; vmax=0.9*vmax; else; vmax=1.2*vmax; end
        han1=plot([tforecast tforecast],[vmin vmax],'--k'); set(han1,'linewidth',2);
        han1=plot([tforecast2 tforecast2],[vmin vmax],'--k'); set(han1,'linewidth',2);
        axis([t0 t1(min(n+tmore,length(t1))) vmin vmax]);
        ...
        nplot=1; subplot('position',[xp0 yp0-(height+ysep)*(nplot-1) width height]); hold on; box on;
        han1=plot(t,Hs(id_tr,:),'-b',t(idw),Hs(id_tr,idw),'ro'); set(han1,'MarkerFaceColor','r','MarkerSize',10);  set(gca,'XTick',linspace(t(1),t(min(n+tmore,length(t))),12),'Xlim',[t(1) t(min(n+tmore,length(t)))]); datetick('x','keeplimits','keepticks');
        ...
        nplot=3; subplot('position',[xp0 yp0-(height+ysep)*(nplot-1) width height]); hold on; box on;
        han1=fill([t_output(~isnan(t_output)) fliplr(t_output(~isnan(t_output)))],[CACI1(id_tr,~isnan(t_output)) fliplr(CACI2(id_tr,~isnan(t_output)))],'r');  set(han1,'FaceAlpha',ALPHA,'EdgeColor','none');
        plot(t_output,CAOUT(id_tr,:),'-r'); set(gca,'XTick',linspace(t1(1),t1(min(n+tmore,length(t1))),12),'Xlim',[t1(1) t1(min(n+tmore,length(t1)))], 'YScale', 'log'); datetick('x','keeplimits','keepticks');
        
        vmin=min(CACI1(id_tr,~isnan(t_output))); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        vmax=max(CACI2(id_tr,~isnan(t_output))); if vmax<0; vmax=0.9*vmax; else; vmax=1.2*vmax; end
        
        if isnan(vmin); vmin=lb_ca; end
        if isnan(vmax); vmax=ub_ca; end
        ...
        ylabel({'$c^{a}$',  '$[m^{1.5}s^{-1}W^{-0.5}]$'}, 'Interpreter', 'latex');
        set(gca,'FontSize',12,'XTickLabel',[]);
        ax = gca; ax.YRuler.Exponent = 0;
        
        nplot=4; subplot('position',[xp0 yp0-(height+ysep)*(nplot-1) width height]); hold on; box on;
        
        han1=fill([t_output(~isnan(t_output)) fliplr(t_output(~isnan(t_output)))],[CECI1(id_tr,~isnan(t_output)) fliplr(CECI2(id_tr,~isnan(t_output)))],'r');  set(han1,'FaceAlpha',ALPHA,'EdgeColor','none');
        plot(t_output,CEOUT(id_tr,:),'-r'); set(gca,'XTick',linspace(t1(1),t1(min(n+tmore,length(t1))),12),'Xlim',[t1(1) t1(min(n+tmore,length(t1)))], 'YScale', 'log'); datetick('x','keeplimits','keepticks');
        
        vmin=min(CECI1(id_tr,~isnan(t_output))); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        vmax=max(CECI2(id_tr,~isnan(t_output))); if vmax<0; vmax=0.9*vmax; else; vmax=1.2*vmax; end
        
        if isnan(vmin); vmin=lb_ce; end
        if isnan(vmax); vmax=ub_ce; end
        ...
        ylabel({'$c^{e}$',  '$[m^{1.5}s^{-1}W^{-0.5}$]'}, 'Interpreter', 'latex');
        set(gca,'FontSize',12,'XTickLabel',[]);
        ax = gca; ax.YRuler.Exponent = 0;
        
        nplot=5; subplot('position',[xp0 yp0-(height+ysep)*(nplot-1) width height]); hold on; box on;
        
        han1=plot([t1(1) t1(min(n+tmore,length(t1)))],[phi0(id_tr) phi0(id_tr)],'--b'); set(han1,'LineWidth',2);
        
        han1=fill([t_output(~isnan(t_output)) fliplr(t_output(~isnan(t_output)))],[PHICI1(id_tr,~isnan(t_output)) fliplr(PHICI2(id_tr,~isnan(t_output)))],'r');  set(han1,'FaceAlpha',ALPHA,'EdgeColor','none');
        plot(t_output,PHIOUT(id_tr,:),'-r'); set(gca,'XTick',linspace(t1(1),t1(min(n+tmore,length(t1))),12),'Xlim',[t1(1) t1(min(n+tmore,length(t1)))], 'YScale', 'log'); datetick('x','keeplimits','keepticks');
        
        vmin=min(PHICI1(id_tr,~isnan(t_output))); vmin=min(vmin,10.^phi0(id_tr)); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        vmax=max(PHICI2(id_tr,~isnan(t_output))); vmax=max(vmax,10.^phi0(id_tr)); if vmax<0; vmax=0.9*vmax; else; vmax=1.2*vmax; end
        
        if isnan(vmin); vmin=lb_phi; end
        if isnan(vmax); vmax=ub_phi; end
        ...
        ylabel({'$\phi$', '(days)'}, 'Interpreter', 'latex');
        set(gca,'FontSize',14,'XTickLabel',[]);
        ax = gca; ax.YRuler.Exponent = 0;
        ...
        plot(t_output,KOUT(id_tr,:),'-r'); set(gca,'XTick',linspace(t1(1),t1(min(n+tmore,length(t1))),12),'Xlim',[t1(1) t1(min(n+tmore,length(t1)))], 'YScale', 'log'); datetick('x','keeplimits','keepticks');
        ...
        ylabel({'$c^{a}$',  '$(m^{1.5}s^{-1}W^{-0.5})$'}, 'Interpreter', 'latex');
        set(gca,'FontSize',12,'XTickLabel',[],'Xlim',[xmin xmax],'Ylim',[vmin vmax], 'YScale', 'log');
        ax = gca; ax.YRuler.Exponent = 0;
        ...
        han1=plot(NTR,phi0,'-b'); set(han1,'LineWidth',2);
        
        han1=plot([ID(id_tr) ID(id_tr)],[vmin vmax],'--r'); set(han1,'linewidth',2);
        
        ylabel({'$\phi$', '(days)'}, 'Interpreter', 'latex');
        set(gca,'FontSize',14,'XTickLabel',[],'Xlim',[xmin xmax],'Ylim',[vmin vmax],'YScale', 'log');
        ax = gca; ax.YRuler.Exponent = 0;
