...
targ_vars = {'CA', 'CE', 'PHI'}; % Target variables
...
for var_idx=1:length(targ_vars)
    ...
    corr_thd = prctile(abs(var_corr), 100-200/(size(var_corr, 1)*size(var_corr, 2))*100, [1,2]); % Use 99 percentile as the threshold
...
for id_tr=1:num_tran
    ...
    for var_idx=1:length(targ_vars)
        ...
        han3=plot(t1, squeeze(var_cali(id_tr, :, :))/dummyC,'-r'); 
        set(han3, 'Color', [1 0 0 0.01]);
        han1=plot(t_output, target/dummyC,'-k');
        set(han1,'linewidth',2);
        han2=plot(t1, model/dummyC,'-r'); 
