...
% model name
Model_name='ShoreFor';  fprintf('Starting CoSMoS-COAST %s model run ... \n',Model_name);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ShoreFor_only = true; % Only consider ShoreFor
Longshore_var = true;
...
tstop=datenum(2017,1,1);        % simulation stop date

tforecast=datenum(2017,01,01);   % the time when data asssimilation is turned off
tforecast2=datenum(2017,01,01);  % the time transition from a hindcast to a forecast (usually when no more data is available)
...
angle=[transects.angle]'; % the transect angle
...
x0=x_on+Y00.*cosd(angle);                 % the x-coordinate of the initial shoreline
y0=y_on+Y00.*sind(angle);                 % the y-coordinate of the initial shoreline
...
xmean=x_on+Ymean.*cosd(angle); % the x-coordinate of the mean shoreline
ymean=y_on+Ymean.*sind(angle); % the y-coordinate of the mean shoreline

% if x/y-mean is still NaN then use the non-erodible position
id=isnan(xmean) | isnan(ymean);
xmean(id)=x_on(id)+Ymin(id).*cosd(angle(id)); % the x-coordinate of the mean shoreline
ymean(id)=y_on(id)+Ymin(id).*sind(angle(id)); % the y-coordinate of the mean shoreline
...
x=repmat(x_on,1,Nens)+Y.*cosd(repmat(angle,1,Nens)); % the x-coordinate of the initial shoreline
y=repmat(y_on,1,Nens)+Y.*sind(repmat(angle,1,Nens)); % the y-coordinate of the initial shoreline
...
% ShoreFor (Crossshore) parameters
phi00 = log10(500); % response factor  [day]
ca00 = 1e-8*sqrt(2229)*24*3600; % accretion rate,  [m^-1.4 day^-1]
ce00 = ca00*0.5;  % erosion rate, units [m^-1.4 day^-1]
...
% noise parameter
sigma00=0; % adative noise [units vary but nominally meters]
...
% ShoreFor parameters (these values are taken from Rai with 3 hours dt, subject to change)
sig_ca = sqrt(1e-6); % Accretion coefficient spread [m^-1.4 day^-1]
sig_ce = sqrt(5e-7); % Erosion coefficient spread [m^-1.4 day^-1]
sig_phi = sqrt(1e-4); % response factor spread [day]
...
% noise parameter
sig_sigma=0.5;  % adative noise spread [units vary but nominally meters]
...
% ShoreFor parameters
phi_min = 5; % Minium value for phi
phi_max = 2000; % Maximum value for phi

phis1 = phi_min:5:95; 
phis2 = 100:10:490;    
phis3 = 500:20:phi_max; 
phis = [phis1, phis2, phis3]; % phis used for pre-calculating F

lb_ca = 0.01; ub_ca = 1.5; % Accretion coefficient  lower and upper bounds (respectively)  [m^-1.4 day^-1]
lb_ce = lb_ca*0.5; ub_ce = ub_ca*0.5+0.05; % Erosion coefficient  lower and upper bounds (respectively)  [m^-1.4 day^-1]
lb_phi = log10(phi_min); ub_phi = log10(phi_max); % response factor  lower and upper bounds (respectively)  [log10(day)]
...
lb_K  =100;   ub_K  =10000;  % longshore transport coefficient lower and upper bounds (respectively)  [-]     (full model transects)

% noise parameter
lb_sigma=-8;   ub_sigma=8;  % adative noise spread [units vary but nominally meters]
...
% make probability distribution objects for ShoreFor parameters
pdf_ca =truncate(makedist('Normal','mu',ca00,'sigma',sig_ca),lb_ca,ub_ca);
pdf_ce =truncate(makedist('Normal','mu',ce00,'sigma',sig_ce),lb_ce,ub_ce);
pdf_phi =truncate(makedist('Normal','mu',phi00,'sigma',sig_phi),lb_phi,ub_phi);
...
% preallocate arrays for the initial conditions of the ShoreFor parameters
ca0=NaN(Ntr,1);
ce0=NaN(Ntr,1);
phi0=NaN(Ntr, 1);
...
% create initial ensembles for ShoreFor parameters
cai =NaN(Ntr,Nens);
cei =NaN(Ntr,Nens);
phii=NaN(Ntr,Nens);
...
for i=1:N_littoral_cells  % for all littoral cells
    ...
    if strcmp(littoral_cell_type(i),'full model') % for 'full model' littoral cells
        ...
        cai(id,:)=repmat(random(pdf_ca,1,Nens),length(id),1); % make alongshore uniform distribution for ca
        cei(id,:)=repmat(random(pdf_ce,1,Nens),length(id),1); % make alongshore uniform distribution for ce
        phii(id,:)=repmat(random(pdf_phi,1,Nens),length(id),1); % make alongshore uniform distribution for phi
...
phii(phii>ub_phi)=ub_phi;
phii(phii<lb_phi)=lb_phi;
...
da_vars           ={'Ylst'  'Yst'   'ca'   'ce'   'phi'   'c'    'vlt'   'K'     'sigma'}; % the names of the variables used in data assimilation
da_var_is_positive=[   0      0      1      1      1       1      0       1       0     ]; % flag to indicate if the data assimilation variable should be forced to be positive
...
da_var_add_noise  =[   0      0      1      1      1       1      1       1       0     ]; % flag to indicate if noise should be added to the data assimilation variable
da_var_noise_fac  =[   0      0    1e-6,    5e-7,  1e-4  0.005    0.01   0.1      0]; % noise factor coefficient
%da_var_noise_fac  =[  1.2    0.5   0.025  0.035  0.0035  0.005    0.01   0.025     0.00025]; % noise factor coefficient
...
if ShoreFor_only
    HOLD_THE_LINE=0; CONTINUED_ACCRETION=1; OUTPUT_DIR='results\ShoreFor_only';
else
    HOLD_THE_LINE=0; CONTINUED_ACCRETION=1; OUTPUT_DIR='results\ShoreFor_CoSMoS';
end
...
if SAVE_DATA
    ...
    CAOUT=NaN(Ntr,len_interval); CAOUT(:,1)=quantile(ca,0.5,2);
    CACI1=NaN(Ntr,len_interval); CACI1(:,1)=quantile(ca,0.025,2)-1e-6; % 95% confidence bands
    CACI2=NaN(Ntr,len_interval); CACI2(:,1)=quantile(ca,0.975,2)+1e-6;

    CEOUT=NaN(Ntr,len_interval); CEOUT(:,1)=quantile(ce,0.5,2);
    CECI1=NaN(Ntr,len_interval); CECI1(:,1)=quantile(ce,0.025,2)-1e-6; % 95% confidence bands
    CECI2=NaN(Ntr,len_interval); CECI2(:,1)=quantile(ce,0.975,2)+1e-6;

    PHIOUT=NaN(Ntr,len_interval); PHIOUT(:,1)=quantile(phi,0.5,2);
    PHICI1=NaN(Ntr,len_interval); PHICI1(:,1)=quantile(phi,0.025,2)-1e-6; % 95% confidence bands
    PHICI2=NaN(Ntr,len_interval); PHICI2(:,1)=quantile(phi,0.975,2)+1e-6;
...
increment = 0;
n = 1;
save_CoSMoS_COAST_output % Save initial conditions with rescaled ShoreFor parameters
...
for n=1:len-1 % for all time steps
    ...
    for nn=1:Nsubcycles  % for all subcycles
        ...
        x=repmat(x_on,1,Nens)+Y.*cosd(repmat(angle,1,Nens)); % the x-coordinate of the shoreline
        y=repmat(y_on,1,Nens)+Y.*sind(repmat(angle,1,Nens)); % the y-coordinate of the shoreline
        ...
        % calculate LST
        % Mao updates the Q calculation to Q~Hs^2.5 considering cg=sqrt(gHs/gamma)
        if HOLD_THE_LINE
            Q=K_mid.*Hs_now_mid.^2.5.*sind(2*alpha_mid).*limiter(beach_width_mid);  % calculate transport at transect midpoints, which is reduced when beach width becomes very narrow
        else
            Q=K_mid.*Hs_now_mid.^2.5.*sind(2*alpha_mid);                            % calculate transport at transect midpoints
        end
    ...
    [~, phi_idx] = min(abs(phi(:) - log10(phis)), [], 2); % Get an index array for the cloest phis used to pre-calculate Fs
    phi_idx = reshape(phi_idx, size(phi));
    [tran_idx, ens_idx] = ndgrid(1:Ntr, 1:Nens);
    time_idx = ones(size(tran_idx))*n;
    linear_idx = sub2ind(size(Fs), phi_idx(:), tran_idx(:), time_idx(:));
    F = reshape(Fs(linear_idx), [Ntr, Nens]);

    % F = NaN(Ntr, Nens); % Preallocate the array for efficiency
    % for tran_id = 1:Ntr
    %     tran_phi = phi(tran_id, :);
    %     for en_id = 1:Nens
    %         [~, ind_phi] = min(abs(tran_phi(en_id) - log10(phis)));
    %         F(tran_id, en_id) = w.Fs.(['phi', num2str(phis(ind_phi))])(WAVEID(WAVEID==tran_id), n);
    %     end
    % end

    dYst = zeros(size(F));
    dYst(F>0) = F(F>0).*ca(F>0)*dt; % acceration [m]
    dYst(F<=0) = F(F<=0).*ce(F<=0)*dt; % erosion [m]
    Yst = Yst + dYst;
        ...
        if  DATA_FOUND % if the current time step is in t_obs (the set of observation times), then we do data assimilation
            ...
            for i=1:N_littoral_cells_with_obs  % loop over all unique littoral cells
                ...
                if SYMBOLIC_MODE
                    ...
                    for k=1:Nens
                        E(:,k)=reshape([Ylst1(:,k) Yst1(:,k) ca1(:,k) ce1(:,k) phi1(:,k) c1(:,k) vlt1(:,k) K1(:,k) sigma1(:,k)]',[],1);  % group all of the transect variables (i.e., the states and parameters) togeather because of the localization part of the Kalman filter operation
                    end
                end
                        ...
                        if SYMBOLIC_MODE % smoothing using the semi-symbolic variables that are assimilated
                            ...
                            elseif strcmp(littoral_cells_with_obs{i},'rate only')                      % for 'rate only' cells
                                if DATA_ASSIMILATION_FOR_RATE_ONLY_TRANSECTS
                                    [~,locb]=ismember({'ca','ce','phi','K'},da_vars(id_smooth_vars));                    % remove 'DT','DY','HSB', and 'K' from needed any smoothing
                                else
                                    [~,locb]=ismember({'ca','ce','phi','K','vlt','c','sigma'},da_vars(id_smooth_vars));  % remove 'DT','DY','HSB', 'K', 'vlt', 'c', and 'sigma' from needed any smoothing
                                end
                                id_smooth_vars(locb)=[];
                            end
                            ...
                            if (strcmp(littoral_cells_with_obs{i},'cross-shore only') || strcmp(littoral_cells_with_obs{i},'full model'))
                                ...
                                ca_new(id_tr_assim,:) =da_var_alpha(strcmp(da_vars,'ca'))*movmean(ca_new(id_tr_assim,:) ,5,1)+(1-da_var_alpha(strcmp(da_vars,'ca')))*ca_new(id_tr_assim,:);
                                ce_new(id_tr_assim,:) =da_var_alpha(strcmp(da_vars,'ce'))*movmean(ce_new(id_tr_assim,:) ,5,1)+(1-da_var_alpha(strcmp(da_vars,'ce')))*ce_new(id_tr_assim,:);
                                dir_new(id_tr_assim,:)=da_var_alpha(strcmp(da_vars,'phi'))*movmean(dir_new(id_tr_assim,:),5,1)+(1-da_var_alpha(strcmp(da_vars,'phi')))*dir_new(id_tr_assim,:);
            ...
            if BOUND
                ...
                for ii=1:Nvars            % for each assimilated variable ...
                    if da_var_bound(ii)   % ... if bounding is turned on
                        if strcmp(da_vars{ii},'ca') || strcmp(da_vars{ii},'ce') || strcmp(da_vars{ii},'phi')
    ...
    if ADDITIVE_NOISE
        ...
        %NOISE=sigma;
        NOISE=ones(size(sigma)); % Mao add this for testing
        ...
        if ADDITIVE_NOISE_LT             % additive noise on the long-term components ... This is turned off when data assimilation is finished
            ...
            id=bool_full_model | bool_cross_shore_only;
            if SYMBOLIC_MODE
                [~,locb]=ismember({'ca','ce','phi'},da_vars);
                ...
                if da_var_add_noise(strcmp(da_vars,'ca'));  ca(id,:) =ca(id,:) +da_var_noise_fac(strcmp(da_vars,'ca'))*repmat(NOISE_FAC(id,:),1,Nens).*NOISE(id,:).*randn(sum(id),Nens);  end
                if da_var_add_noise(strcmp(da_vars,'ce'));  ce(id,:) =ce(id,:) +da_var_noise_fac(strcmp(da_vars,'ce'))*repmat(NOISE_FAC(id,:),1,Nens).*NOISE(id,:).*randn(sum(id),Nens);  end
                if da_var_add_noise(strcmp(da_vars,'phi')); phi(id,:)=phi(id,:)+da_var_noise_fac(strcmp(da_vars,'phi'))*repmat(NOISE_FAC(id,:),1,Nens).*NOISE(id,:).*randn(sum(id),Nens); end
            end
...
if SAVE_DATA
    ...
    save(matname2,'t_output','ca','ce','phi','c','K','sigma','vlt','vlt_assim','CAOUT','CACI1','CACI2','CEOUT','CECI1','CECI2', 'PHIOUT','PHICI1','PHICI2','KOUT','KCI1','KCI2','COUT','CCI1','CCI2','VLTOUT','VLTCI1','VLTCI2','LTER','SIGMAOUT','SIGMACI1','SIGMACI2','sigma0','sigma00','da_vars','da_var_is_positive','da_var_smooth','da_var_alpha','da_var_bound','da_var_add_noise','da_var_noise_fac','-v7.3');
