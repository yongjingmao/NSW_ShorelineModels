...
matlab_helpers_toolbox='/Users/z3392050/Library/CloudStorage/OneDrive-UNSW/Research/ARC_FutureFellowship/Shoreline Modelling/CosMoS-Coast/MatlabToolbox';
...
%WAVES_FILE='../waves/wavesSydMeshOnly_BMU.mat'; %load NEW bias corrected waves .
WAVES_FILE='../waves/waves.mat'; %load NEW bias corrected waves .

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Setup model time stepping
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
t0=datenum(1987,1,1);           % simulation start date

%tstop=datenum(2100,1,1);       % simulation stop date
tstop=datenum(2023,6,1);        % simulation stop date
...
tforecast=datenum(2014,01,01);   % the time when data asssimilation is turned off
%tforecast=datenum(2019,01,01);   % the time when data asssimilation is turned off
tforecast2=datenum(2019,01,01);  % the time transition from a hindcast to a forecast (usually when no more data is available)
...
% ids=find(bbox==206);
%ids=find(strcmp({transects.littoral_cell}','Curl Curl'));

ids=[transects(ids).ID]';
...
if ~exist('Hs','var')
    ...
    if 1
        ...
        % transect angle in nautical convention
        phi_naut=mod(270-(phi-180),360);

        % get mean direction for all times
        Dir_mean=nanmean(Dir,2);

        % remove the mean direction and add back the transect direction, so
        % that the wave direction (on average) is the same as the transect angle
        Dir=Dir-Dir_mean+phi_naut;
...
DT00=60;   % equlibrium    time scale [days]
DY00=7;   % equlibrium erosion scale [meters]       (for full model transects)
...
% long-term parameters
sig_c  =0.1;  % Bruun coefficient spread        [-]
sig_vlt=0.01; % long-term rate term             [m/yr]
...
lb_vlt=-2;     ub_vlt=2;    % long-term rate term             lower and upper bounds (respectively)  [m/yr]
lb_K  =0.01;   ub_K  =1500; % longshore transport coefficient lower and upper bounds (respectively)  [-]     (full model transects)
...
da_var_noise_fac  =[  1.2    0.5   0.025  0.025  0.0025  0.005    0.005   0.025     0.00025]; % noise factor coefficient
...
if ~exist('Y_obs','var')
    ...
    mhw=0.0;  % approximate mean high water level height above mean sea level for Duck, NC
    msl=0;
...
HOLD_THE_LINE=0; CONTINUED_ACCRETION=1; OUTPUT_DIR='results/ManagmentCase4_no_hold_the_line_continued_accretion_SydMeshOnly_CorrectedWaves';
...
if SAVE_DATA
    ...
    % keep track of cumulative transport
    Qcum=zeros(Ntr+1,Nens);

    QCUMOUT=NaN(Ntr+1,len_interval); QCUMOUT(:,1)=quantile(Qcum,0.5,2);
    QCUMOUTCI1=NaN(Ntr+1,len_interval); QCUMOUTCI1(:,1)=quantile(Qcum,0.025,2)-1e-6; % 95% confidence bands
    QCUMOUTCI2=NaN(Ntr+1,len_interval); QCUMOUTCI2(:,1)=quantile(Qcum,0.975,2)+1e-6;
...
for n=1:len-1 % for all time steps
    ...
    for nn=1:Nsubcycles  % for all subcycles
        ...
        beach_width_mid=0.5*(beach_width(2:end,:)+beach_width(1:end-1,:));
        
        % K_mid=(alpha_mid>0).*K(1:end-1,:)+(alpha_mid<0).*K(2:end,:)+(alpha_mid==0).*(0.5*(K(2:end,:)+K(1:end-1,:)));
        K_mid=0.5*(K(2:end,:)+K(1:end-1,:));
    ...
    dSdt=(S(n+1)-S(n))/dt;                                                            % the rate of SLR at the current time step
...
if SAVE_DATA
    ...
    save(matname1,'x0','y0','Y00','Ymin','Ntr','ID','HOLD_THE_LINE','CONTINUED_ACCRETION','Model_name','UTMZONE','SL','t1','t0','tstop','tforecast','tforecast2','dt','Nsubcycles','y_rms_sat','y_rms_gps','t_obs','Y_obs','Y_rms','SAT','S','Smean','t_output','Y','Ylst','Ylt','Yst','Ybru','Yvlt','Y0','YY','YST','YLST','YBRU','YVLT','YCI1','YCI2','YSTCI1','YSTCI2','YLSTCI1','YLSTCI2','YBRUCI1','YBRUCI2','YVLTCI1','YVLTCI2','NOBS','NOBS_cal','NOBS_val','R','MSE','RMSE','RMSM','MAE','SKILL','IDX_AGREEMENT','LAMBDA','NOBS_cal_sat','NOBS_val_sat','NOBS_sat','R_sat','MSE_sat','RMSE_sat','RMSM_sat','MAE_sat','SKILL_sat','IDX_AGREEMENT_sat','LAMBDA_sat','PCT_sat','-v7.3');
...
geolimits([min(lat_on_NSW)-0.1 max(lat_on_NSW)+0.1],[min(lon_on_NSW)-0.1 max(lon_on_NSW)+0.1]);
for i=round(min(find(strcmp(STATE,'NSW')))):100:max(find(strcmp(STATE,'NSW')));
...
for i=1:N_littoral_cells
    if littoral_cell_tr_length(i)>20
...
set(gca,'FontSize',14,'layer','top','Xtick',XMIN:20:XMAX,'Ytick',round(min(find(strcmp(STATE,'NSW'))),-3):100:max(find(strcmp(STATE,'NSW'))),'YAxisLocation','right','YDir','reverse');
xlabel('RMSE SAT (m)')
