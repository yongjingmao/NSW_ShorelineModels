...
for i=1:length(ids)
    ...
    if (strcmp(transects(id_tr).model_type,'full model') || strcmp(transects(id_tr).model_type,'cross-shore only') || strcmp(transects(id_tr).model_type,'rate only')) && ~bool_no_prediction(id_tr)
        
        fprintf('working on transect ID # %d (%d of %d) ... \n',ID(id_tr),i,length(ids)); % display progress
        ...
        hanf1=figure(f1); set(hanf1,'PaperPositionMode','Auto','Position',[50 100 920 1000],'color','w');
        ...
        %han1=plot(t(idw),Hs(id_tr,idw),'ro'); set(han1,'MarkerFaceColor','r','MarkerSize',10);  
        set(gca,'XTick',XTICK,'XLim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        han1=plot([t0 tstop],[0 0],'k--'); set(han1,'HandleVisibility','off')
        ...
        plot(t_output,YY(id_tr,:)-Y00(id_tr),'-r'); 
        set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        plot(t_output,YST(id_tr,:),'-r'); 
        set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');

        %vmin=min(YSTCI1(id_tr,~isnan(t_output))); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        vmin=min(YST(id_tr,~isnan(t_output))); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        %vmin=-75;
        %vmax=max(YST(id_tr,~isnan(t_output))); if vmax<0; vmax=0.9*vmax; else; vmax=1.2*vmax; end
        ...
        vmin=min(YLSTCI1(id_tr,~isnan(t_output))); if vmin<0; vmin=1.2*vmin; else; vmin=0.9*vmin; end
        vmax=max(YLSTCI2(id_tr,~isnan(t_output))); if vmax<0; vmax=0.9*vmax; else; vmax=1.2*vmax; end
        han1=plot([tforecast tforecast],[vmin vmax],'--k'); set(han1,'linewidth',2);
        han1=plot([tforecast2 tforecast2],[vmin vmax],'--k'); set(han1,'linewidth',2);
        set(gca,'YLim',[vmin vmax]);
        ...
        hanf2=figure(f2); set(hanf2,'PaperPositionMode','Auto','Position',[985 100 920 1000],'color','w');
        ...
        nplot=1; subplot('position',[xp0 yp0-(height+ysep)*(nplot-1) width height]); hold on; box on;
        han1=plot(t,Hs(id_tr,:),'-b',t(idw),Hs(id_tr,idw),'ro'); set(han1,'MarkerFaceColor','r','MarkerSize',10);  set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        han1=fill([t_output(~isnan(t_output)) fliplr(t_output(~isnan(t_output)))],[YCI1(id_tr,~isnan(t_output))-Y00(id_tr) fliplr(YCI2(id_tr,~isnan(t_output))-Y00(id_tr))],'r');  set(han1,'FaceAlpha',ALPHA,'EdgeColor','none');
        plot(t_output,YY(id_tr,:)-Y00(id_tr),'-r'); set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        plot(t_output,DTOUT(id_tr,:),'-r');   set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        plot(t_output,DYOUT(id_tr,:),'-r');   set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        plot(t_output,VLTOUT(id_tr,:),'-r');   set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        plot(t_output,SIGMAOUT(id_tr,:),'-r');   set(gca,'XTick',XTICK,'Xlim',XLIM); datetick('x','keeplimits','keepticks');
        ...
        hanf3=figure(f3); set(hanf3,'PaperPositionMode','Auto','Position',[50 100 920 1000],'color','w');
        ...
        hanf4=figure(f4); set(hanf4,'PaperPositionMode','Auto','Position',[100 100 920 700],'color','w');
        ...
        nn=~isnan(Y_obs(id_tr,:)) & SAT(id_tr,:)==0;
        han1=errorbar(t_obs(nn),Y_obs(id_tr,nn)-Y00(id_tr),2*Y_rms(id_tr,nn),'m'); set(han1,'linestyle','none','color',[0.278 0 0.7],'HandleVisibility','off');
        han1=plot(t_obs(nn),Y_obs(id_tr,nn)-Y00(id_tr),'mo'); set(han1,'MarkerSize',4,'MarkerFaceColor',[0.58 0.30 1],'MarkerEdgeColor',[0.278 0 0.7],'HandleVisibility','off'); datetick('x','keeplimits','keepticks')
        ...
        han1=legend('observed C.I.','observed','model C.I.','model');
        set(han1,'Position',[0.0952672108219599 0.0953616167064611 0.185869561330132 0.127857139280864]);
        ...
        % saveas(figure(f1),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_results_components_tr_ID_',num2str(ID(id_tr),'%0.5d'),'.png'],'png');
        % saveas(figure(f2),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_results_parameters_tr_ID_',num2str(ID(id_tr),'%0.5d'),'.png'],'png');
        % saveas(figure(f3),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_results_alongshore_tr_ID_',num2str(ID(id_tr),'%0.5d'),'.png'],'png');
        % saveas(figure(f4),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_calibration_validation_tr_ID_',num2str(ID(id_tr),'%0.5d'),'.png'],'png');
        % 
        saveas(figure(f1),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_results_components_tr_ID_',transects(ID(id_tr)).coastsat_tr_name,'.png'],'png');
        saveas(figure(f2),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_results_parameters_tr_ID_',transects(ID(id_tr)).coastsat_tr_name,'.png'],'png');
        saveas(figure(f3),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_results_alongshore_tr_ID_',transects(ID(id_tr)).coastsat_tr_name,'.png'],'png');
        saveas(figure(f4),[OUTPUT_DIR,filesep,'figures',filesep,'CoSMoS-COAST_calibration_validation_tr_ID_',transects(ID(id_tr)).coastsat_tr_name,'.png'],'png');
