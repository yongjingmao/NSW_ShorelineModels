...
transectFn = 'TransectNames_NSW_KS_SV_SydMeshOnly.xls';
...
if ismac
    DATA_DIR='/Users/z3392050/Library/CloudStorage/OneDrive-UNSW/Research/ARC_FutureFellowship/Shoreline Modelling/DATA/NSW_Regional/CoastSatShorelines/NSW_updated/csv/csv_v7';
...
T=readtable(transectFn); %can edit this file to remove files/transects that may not be useful, etc. 
sites=T.ID;
rank=T.RANK;
name=T.Name;
...
for i=1:length(sites)  % for all data folders
    ...
    data_file=[DATA_DIR,filesep,all_sites{id},filesep,'transect_coordinates_and_beach_slopes_Landsat.csv']; % data file to load
...
SAVE_TRANSECTS=1;
if SAVE_TRANSECTS
    save('transects_SYD','transects','-v7.3'); % save in current folder
end

% get littoral cell information from the transects
[littoral_cell_name,littoral_cell_name_unique,littoral_cell_type,...
